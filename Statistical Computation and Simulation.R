...
# generating random samples of distribution using Probabaility Integral Transformation:
...
# Another alternative is using the density function which uses (by default gaussian) kernels
# to estimate the density of the sample
...
# Visualizing the method of accepting (green dots) and rejecting (red dots) at some specified
# segments
...
# the higher the density of f the more points are accepted
# however if cg(x) >> f(x) we will have lots of rejections,which will decrease the 
# quality of simulation
# for same amount of points, if c = 10
...
# this number of pints is bot enough to get an estimate with the quality of previou example
